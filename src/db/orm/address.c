...
SQL_RESULT* address_execute_find(char const* query, MYSQL_BIND* params, uint param_count) {
	...
	while (!mysql_stmt_fetch(stmt)) {
		...
		if (is_null[1]) {
			...
			((ADDRESS*) row->data)->municipality = municipality_find_by_id(id_municipality_buffer);
		}
		if (is_null[2]) {
...
int address_update(ADDRESS* addressT) {
	...
	retval = address_execute(QUERY, param, PARAM_COUNT);
