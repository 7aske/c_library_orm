...
uint municipality_insert(MYSQL* conn, MUNICIPALITY* municipalityT) {
	...
	if (municipalityT->region == NULL){
		...
		region_insert(conn, municipalityT->region);
	} else {
		region_update(conn, municipalityT->region);
	}
...
SQL_RESULT* municipality_execute_find(MYSQL* conn, char const* query, MYSQL_BIND* params, uint param_count) {
	...
	/* INTEGER COLUMN */
	param[0].buffer_type = MYSQL_TYPE_LONG;
	param[0].buffer = &id_municipality_buffer;
	...
	param[1].buffer = &id_region_buffer;
	...
	while (!mysql_stmt_fetch(stmt)) {
		...
		if (is_null[0]) {
			...
			((MUNICIPALITY*) row->data)->id_municipality = id_municipality_buffer;
		}
		if (is_null[1]) {
			((MUNICIPALITY*) row->data)->region = NULL;
		} else {
			((MUNICIPALITY*) row->data)->region = region_find_by_id(conn, id_region_buffer);
		}
		if (is_null[2]) {
			strcpy(((MUNICIPALITY*) row->data)->name, "NULL");
		} else {
			strncpy(((MUNICIPALITY*) row->data)->name, name_buffer, lengths[2]);
		}
	}
...
MUNICIPALITY* municipality_find_by_id(MYSQL* conn, uint id) {
	...
	municipality.id_municipality = id;
	struct municipality* municipalityT = &municipality;
	...
	param[0].buffer = malloc(sizeof(uint));
	param[0].buffer_type = MYSQL_TYPE_LONG;
	memcpy(param[0].buffer, &municipalityT->id_municipality, sizeof(uint));

	res = municipality_execute_find(conn, QUERY, param, PARAM_COUNT);
...
int municipality_update(MYSQL* conn, MUNICIPALITY* municipalityT) {
	#define QUERY "update municipality set id_region = ?, name = ? where id_municipality = ?;"
	...
	memcpy(param[2].buffer, &municipalityT->id_municipality, sizeof(uint));

	retval = municipality_execute(conn, QUERY, param, PARAM_COUNT);
...
int municipality_delete(MYSQL* conn, MUNICIPALITY* municipalityT) {
	#define QUERY "delete from municipality where id_municipality = ?;"
	#define PARAM_COUNT 1
	assert(municipalityT->id_municipality != 0);
